...
class LLMRequest(BaseModel):
    ...
    temperature: float = Field(default=0.1, ge=0.0, le=1.0, description="Response temperature")
...
class LLMResponse(BaseModel):
    ...
    stop_reason: Optional[str] = Field(default=None, description="Reason generation stopped")
...
class LLMClient:
    ...
    def __init__(self, model: str = None, max_tokens: int = None,
        ...
        try:
            ...
            self.max_tokens = max_tokens if max_tokens is not None else config.get_int("max_tokens", 4096)
            self.temperature = temperature if temperature is not None else config.get_float("temperature", 0.1)
            self.timeout_seconds = timeout_seconds if timeout_seconds is not None else config.get_int("timeout_seconds", 300)
    ...
    async def generate_text(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            logger.info(f"Generating text with model: {self.model}, prompt length: {len(request.prompt)}")
                ...
                try:
                    ...
                    if total_cost and total_cost > 0:
                        ...
                        if total_tokens > 0:
                            ...
                            usage["completion_cost_usd"] = completion_tokens * cost_per_token
                    ...
                    if hasattr(response.usage, 'completion_tokens_cost_usd'):
                        usage["completion_cost_usd"] = response.usage.completion_tokens_cost_usd
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = litellm.completion(
                model=self.model,
                messages=[{"role": "user", "content": "Hello, please respond with 'Connection successful' to test the API."}],
                max_tokens=50,
                temperature=0.1,
                timeout=30
            )
...
def get_llm_client(model: str = None, max_tokens: int = None,
                   temperature: float = None, timeout_seconds: int = None,
                   debug: bool = None) -> LLMClient:
    ...
    actual_max_tokens = max_tokens if max_tokens is not None else config.get_int("max_tokens", 4096)
    actual_temperature = temperature if temperature is not None else config.get_float("temperature", 0.1)
    actual_timeout = timeout_seconds if timeout_seconds is not None else config.get_int("timeout_seconds", 300)
    ...
    async def test_llm_client():
        ...
        try:
            ...
            if connection_ok:
                ...
                logger.info(f"Generated response from {response.model}: {response.text[:200]}...")

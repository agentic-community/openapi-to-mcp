...
if __name__ == "__main__":
    ...
    try:
        ...
        if config.provider_name != "anthropic":
            all_validation_failures.append(
                f"Basic creation: Expected provider_name 'anthropic', got '{config.provider_name}'"
            )
        if config.spec_name != "petstore":
            all_validation_failures.append(
                f"Basic creation: Expected spec_name 'petstore', got '{config.spec_name}'"
            )
        if config.timestamp != "20240106_143022":
            all_validation_failures.append(
                f"Basic creation: Expected timestamp '20240106_143022', got '{config.timestamp}'"
            )
    except Exception as e:
        all_validation_failures.append(
            f"Basic creation: Unexpected exception {type(e).__name__}: {e}"
        )
    ...
    try:
        ...
        if results_dir != expected_dir:
            all_validation_failures.append(
                f"Results directory: Expected '{expected_dir}', got '{results_dir}'"
            )
    except Exception as e:
        all_validation_failures.append(
            f"Results directory: Unexpected exception {type(e).__name__}: {e}"
        )
    ...
    try:
        ...
        eval_file, enhanced_file, original_file, summary_file, usage_file = (
            config.get_output_files()
        )

        expected_eval = Path("results/claude/api/evaluation_20240202_180000.json")
        if eval_file != expected_eval:
            all_validation_failures.append(
                f"Output files: Expected eval_file '{expected_eval}', got '{eval_file}'"
            )

        expected_enhanced = Path(
            "results/claude/api/enhanced_spec_20240202_180000.yaml"
        )
        if enhanced_file != expected_enhanced:
            all_validation_failures.append(
                f"Output files: Expected enhanced_file '{expected_enhanced}', got '{enhanced_file}'"
            )

        expected_summary = Path("results/claude/api/summary_20240202_180000.md")
        if summary_file != expected_summary:
            all_validation_failures.append(
                f"Output files: Expected summary_file '{expected_summary}', got '{summary_file}'"
            )
    except Exception as e:
        all_validation_failures.append(
            f"Output files: Unexpected exception {type(e).__name__}: {e}"
        )
    ...
    try:
        ...
        if not expected_dir.exists():
            all_validation_failures.append(
                "Directory creation: Results directory was not created by get_output_files()"
            )
    except Exception as e:
        all_validation_failures.append(
            f"Directory creation: Unexpected exception {type(e).__name__}: {e}"
        )

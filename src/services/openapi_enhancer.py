...
from ..models.evaluation import OpenAPIEvaluationResult, LintingResult, LintingIssue, LintingSeverity
...
class OpenAPILinter:
    ...
    def lint_specification(self, spec_dict: Dict[str, Any]) -> LintingResult:
        ...
        try:
            ...
            error_count = len([i for i in self.issues if i.severity == LintingSeverity.ERROR])
            warning_count = len([i for i in self.issues if i.severity == LintingSeverity.WARNING])
            info_count = len([i for i in self.issues if i.severity == LintingSeverity.INFO])
            suggestion_count = len([i for i in self.issues if i.severity == LintingSeverity.SUGGESTION])
            
            # Calculate linting score (1-5 scale)
            linting_score = self._calculate_linting_score(error_count, warning_count, info_count)
            
            # Generate summary
            summary = self._generate_linting_summary(error_count, warning_count, info_count, suggestion_count)
            ...
            logger.info(f"Linting completed: {len(self.issues)} issues found (Score: {linting_score}/5)")
            return result
    ...
    def _add_issue(self, severity: LintingSeverity, rule: str, message: str, 
                   path: Optional[str] = None, location: Optional[str] = None, 
    ...
    def _check_basic_structure(self, spec: Dict[str, Any]):
        ...
        if "openapi" in spec:
            ...
            if not isinstance(version, str):
                self._add_issue(LintingSeverity.ERROR, "openapi-version", "OpenAPI version must be a string")
    ...
    def _check_paths(self, spec: Dict[str, Any]):
        ...
        if not isinstance(paths, dict):
            self._add_issue(LintingSeverity.ERROR, "paths-invalid", "Paths must be an object")
            return
        ...
        if "schemas" in components:
            ...
            if isinstance(schemas, dict):
                for schema_name, schema in schemas.items():
                    self._check_schema(f"$.components.schemas.{schema_name}", schema_name, schema)
    ...
    def _check_security(self, spec: Dict[str, Any]):
        ...
        has_security_schemes = "components" in spec and "securitySchemes" in spec["components"]
    ...
    def _calculate_linting_score(self, error_count: int, warning_count: int, info_count: int) -> int:
    ...
    def _generate_linting_summary(self, error_count: int, warning_count: int, 
                                 info_count: int, suggestion_count: int) -> str:
        ...
        if error_count > 0:
            summary += " Address errors first as they may prevent proper API functionality."
...
class EnhancementRequest(BaseModel):
    ...
    spec_content: str = Field(description="OpenAPI specification content (JSON or YAML)")
    spec_format: str = Field(default="yaml", description="Format of the spec (json or yaml)")
    original_filename: Optional[str] = Field(default=None, description="Original filename")
    original_url: Optional[str] = Field(default=None, description="Original URL if from web")
    enhancement_level: str = Field(default="comprehensive", description="Level of enhancement")
...
class EnhancementResult(BaseModel):
    ...
    enhanced_spec: Optional[str] = Field(default=None, description="Enhanced specification")
    enhancement_summary: Dict[str, Any] = Field(description="Summary of enhancements made")
...
class OpenAPIEnhancer:
    ...
    def _load_evaluation_template(self) -> Template:
        ...
        try:
            ...
            if not template_path.exists():
                raise FileNotFoundError(f"Evaluation template not found: {template_path}")
    ...
    def _parse_openapi_spec(self, content: str, format_hint: str = "yaml") -> Dict[str, Any]:
        ...
        try:
            ...
            if "openapi" not in spec_dict and "swagger" not in spec_dict:
                raise ValueError("Content does not appear to be an OpenAPI specification")
    ...
    def _extract_spec_metadata(self, spec_dict: Dict[str, Any]) -> Dict[str, str]:
        """Extract basic metadata from OpenAPI specification."""
        try:
    ...
    async def evaluate_specification(self, request: EnhancementRequest) -> OpenAPIEvaluationResult:
        """Evaluate an OpenAPI specification for completeness and AI readiness."""
        try:
            logger.info(f"Starting evaluation for {request.original_filename or 'spec'}")
            
            # Parse the specification
            spec_dict = self._parse_openapi_spec(request.spec_content, request.spec_format)
            ...
            logger.info(f"LLM inference parameters - max_tokens: {max_tokens}, temperature: {temperature}")
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', prefix='llm_response_', delete=False) as temp_file:
            ...
            if hasattr(response, 'usage') and response.usage:
                ...
                if total_cost > 0:
                    logger.info(f"💰 Cost breakdown - prompt: ${prompt_cost:.6f}, completion: ${completion_cost:.6f}, total: ${total_cost:.6f}")
                else:
                    logger.info("💰 Cost information not available from provider")
                
                # Check if response might be truncated
                if completion_tokens >= max_tokens * 0.95:
                    logger.warning(f"⚠️ Response may be truncated! Completion tokens ({completion_tokens}) near max_tokens limit ({max_tokens})")
            else:
            ...
            try:
                ...
                logger.info(f"Extracted JSON content from position {json_start} to {json_end}")
                ...
                try:
                    ...
                    logger.error(f"JSON parsing failed at line {json_err.lineno}, column {json_err.colno}")
                    logger.error(f"JSON error message: {json_err.msg}")
                    logger.error(f"JSON content around error (chars {max(0, json_err.pos-50)}:{json_err.pos+50}): {json_content[max(0, json_err.pos-50):json_err.pos+50]}")
                    raise
                
                # No longer extracting enhanced spec since we removed it from the prompt
                logger.info("Enhanced spec generation removed from prompt - focusing on evaluation only")
                ...
                logger.info(f"Evaluation evaluation_usage: {evaluation.evaluation_usage}")
                logger.info(f"Successfully evaluated spec: {evaluation.api_title}")
                logger.info(f"Linting score: {linting_results.linting_score}/5 ({linting_results.total_issues} issues)")
                return evaluation
    ...
    async def enhance_specification(self, request: EnhancementRequest) -> EnhancementResult:
        ...
        try:
            ...
            enhancement_summary = {
                ...
                "enhancement_applied": bool(enhanced_spec),  # True if enhanced spec was provided
                ...
                "major_improvements_identified": len(evaluation.overall.major_improvements_needed),
                "minor_improvements_identified": len(evaluation.overall.minor_improvements_suggested),
                "enhanced_spec_length": len(enhanced_spec) if enhanced_spec else 0,
                "linting_score": evaluation.linting_results.linting_score if evaluation.linting_results else None,
                "linting_issues_total": evaluation.linting_results.total_issues if evaluation.linting_results else None,
                "linting_errors": evaluation.linting_results.error_count if evaluation.linting_results else None,
                "linting_warnings": evaluation.linting_results.warning_count if evaluation.linting_results else None
            }
            ...
            logger.info(f"Completed enhancement process for request {request_id} in {processing_time:.2f}s")
            return result
...
"""
            ...
            if is_valid:
                ...
                try:
                    ...
                    logger.warning(f"Enhancement failed (expected without AWS credentials): {e}")
